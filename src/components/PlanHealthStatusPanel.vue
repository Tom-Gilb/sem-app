...
// Both helpers live in usePlanHealthExport.ts so the format stays identical
...
function makeExportInput(): {
  ...
  custom: ReturnType<typeof usePlanHealth>['custom']['value']
  by: string
} {
  return {
  ...
  >
    ...
    <div class="px-5 py-3 flex items-center gap-3 shrink-0
      ...
      <button
        type="button"
        class="text-[11px] px-2 py-1 rounded bg-white/15 hover:bg-white/25 text-white font-semibold transition-colors"
        title="Open the Plan Health Record Administration Specification (weights, notifications, snapshots)"
