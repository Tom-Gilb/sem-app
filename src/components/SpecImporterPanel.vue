...
const PLAN_IMPORTER_WISDOM = [
  ...
  },
  {
    ...
    text: 'A Constraint Spec uses Must / Must not form. It is binary — either respected or violated. No option that violates a constraint is acceptable, regardless of its Value score. Budget, legal, and safety rules are always Constraint Specs.',
...
<template>
  <Teleport to="body">
    ...
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Spec Agent — universal Planguage converter and improvement loop"
    >
      ...
      <div class="flex items-center gap-3 px-5 py-4 bg-gradient-to-r from-orange-700 to-orange-600 shrink-0 select-none">
        ...
        <div class="flex-1 min-w-0">
          <h2 class="text-base font-bold text-white leading-tight tracking-tight">Spec Agent</h2>
        ...
        <CloseDot
          variant="on-dark"
          title="Close Spec Agent — return to the main planning workspace"
          @click="emit('close')"
        />
      </div>
          ...
          <div class="h-full flex items-center justify-center px-8">
            <div class="max-w-lg w-full">
              ...
              <div class="border-2 border-dashed border-orange-200 rounded-2xl p-8 bg-orange-50/50 text-center mb-6">
                ...
                <p class="text-xs text-slate-500 mb-4">AI will convert it to full Planguage Function / Value / Constraint / Resource / Solution Specs and analyse for problems</p>
              ...
              <ScrollContainer outer-class="flex-1 min-h-0 relative" inner-class="px-3 py-2 space-y-2">
                ...
                <div class="border-t border-slate-200 pt-3">
                  ...
                  <textarea
                    ...
                    placeholder="Type an improvement order… e.g. 'simplify all Value Specs', 'add innovation-focused Function Specs', 'make first Value Goal more ambitious', 'add missing constraint for data privacy'"
                  ...
                  <div class="flex flex-wrap gap-1 mb-2">
                    ...
                    <button
                      type="button"
                      class="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-orange-100 hover:bg-orange-200 text-orange-700 transition-colors"
                      title="Add missing Value Specs — identify implied values not yet captured"
                      @click="setQuickCommand('Add missing Value Specs — identify all implied values not yet captured as explicit Planguage Value Specs')"
                    >
                      Add Values
                    </button>
                    <button
                      type="button"
                      class="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-orange-100 hover:bg-orange-200 text-orange-700 transition-colors"
                      title="Tighten constraints — make all Constraint Specs more precise with Must/Must not form"
                      @click="setQuickCommand('Tighten all Constraint Specs — ensure each uses the Must/Must not form and has a clear scope')"
                    >
                      Tighten Constraints
                    </button>
                    <button
                      type="button"
                      class="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-orange-100 hover:bg-orange-200 text-orange-700 transition-colors"
                      title="Make measurable — add Scale, Meter, Goal, Tolerable to all Value Specs that are missing them"
                      @click="setQuickCommand('Make all Value Specs fully measurable — add Scale, Meter, Goal, and Tolerable to any Value Spec that is missing them')"
                    >
                      Make Measurable
                    </button>
                  </div>
