<!-- UNIT_TYPE=Surface
  IncorruptibleSharpeningPanel.vue — Question/answer Sharpening tool for the Incorruptible Agent.

  Tom Gilb 2026-06-11 (verbatim):
    "when I said sharpening, I meant with generate questions and answers based on the plan and on
     incorruptible standards, like we do elsewhere w sharpening, but tailored to this agent"
    "Incorruptible Sharpening: is name of the tool within the agent and available outside"
    "make it so, now thank you"

  Architecture:
    - Six categories matching the Incorruptible finding categories (Quarterly Tyranny / Stakeholder
      Monoculture / Mission Drift / Founder-Vision Erosion / Innovation-Budget Predation / Governance Hole)
    - Per category: 2 pointed questions, each with 3 AI-suggested starter answers + provenance badges
    - User answers + selections → synthesise IncorruptibleFinding[] via useIncorruptibleSharpSynthesis
    - Synthesised findings emit via 'synthesise-findings' event — host (App.vue) applies them via
      the existing applyIncorruptibleFix infrastructure → source stamping (r93s) + accept state
      (r93u) + Universal Undo (r93v) all just work

  UI Rules applied:
    - CloseDot at END of header (rightmost) — Universal Close-Button Rule
    - Raw overflow-y-auto on body — narrow exception per r93r (centered Teleport card pattern)
    - z-[490] panel / z-[485] backdrop — Major surfaces tier
    - All buttons have title= — DD-009 Interaction Disclosure
    - Big fonts per accessibility_tom.md (text-sm minimum content, 11px minimum labels)
    - R/G-colorblind-safe palette per r93o
    - Scroll-engagement verified per r93t: panel body is `flex-1 min-h-0 overflow-y-auto`
-->
...
import {
  INCORRUPTIBLE_SHARP_CATEGORIES,
  totalIncorruptibleSharpQuestions,
  type IncorruptibleSharpCategory,
} from '../data/incorruptibleSharpInterview'
import {
  synthesiseIncorruptibleFindings,
  type AnsweredQuestion,
} from '../composables/useIncorruptibleSharpSynthesis'
import { generatePlanDerivedSuggestions } from '../composables/useIncorruptibleSharpPlanContext'
import type { IncorruptibleFinding, IncorruptibleCategory } from '../types/incorruptible'
import {
  copyUniversalSharp,
  emailUniversalSharp,
...
const props = withDefaults(defineProps<{
  spec: SpecBlock | null
  planTitle: string
  /** True when sharpening a Library model (not the user's current Plan).
   *  Tom: "Incorruptible Sharpening: is name of the tool within the agent and available outside". */
  isModel?: boolean
}>(), {
  isModel: false,
})
...
const emit = defineEmits<{
  close: []
  /** Emitted when user clicks "Synthesise & Apply" — payload is the array of synthesised findings.
   *  Host routes each through applyIncorruptibleFix via onIncorruptibleAcceptFix. */
  'synthesise-findings': [findings: IncorruptibleFinding[]]
}>()

// ─── r93bb Phase 2b: plan-derived suggestions ─────────────────────────────
// Run the deterministic detectors against the bound spec; produce a Map keyed by
// questionId → plan-derived starter answer. These render as a 4th option above the
// 3 static suggestions, with a 'plan' provenance badge (highest provenance per
...
// ─── r93bb Phase 2b: localStorage persistence of answers ──────────────────
// Tom: "I want it all" → answers persist across sessions per plan-title so the user
// can leave mid-flow and return. Key: incorruptibleSharp:answers:<planTitle>
const STORAGE_KEY_PREFIX = 'incorruptibleSharp:answers:'
...
interface QAState {
  /** Free-text answer the user typed. */
  answer: string
  /** Indexes of static suggested answers the user has ticked. */
  selectedSuggestionIndexes: number[]
  /** r93bb Phase 2b: whether the plan-derived suggestion (when present) is selected. */
  planDerivedSelected?: boolean
}

/** Map keyed by "categoryId|questionId" → current QAState for that question. */
...
/** Reset all answers (called explicitly via the "Clear All Answers" button). */
function clearAllAnswers(): void {
  if (!confirm('Clear all your Sharpening answers for this plan? This cannot be undone.')) return
...
const activeCategoryId = ref<IncorruptibleSharpCategory['id']>(INCORRUPTIBLE_SHARP_CATEGORIES[0].id)
const activeCategory   = computed<IncorruptibleSharpCategory>(
  () => INCORRUPTIBLE_SHARP_CATEGORIES.find(c => c.id === activeCategoryId.value) ?? INCORRUPTIBLE_SHARP_CATEGORIES[0],
)

/** Progress per category — { answeredQuestionCount, totalQuestionCount }. */
function categoryProgress(cat: IncorruptibleSharpCategory): { answered: number; total: number } {
  let answered = 0
...
const totalAnswered = computed<number>(() => {
  let n = 0
  for (const cat of INCORRUPTIBLE_SHARP_CATEGORIES) {
    n += categoryProgress(cat).answered
  }
  return n
})

const totalQuestions = totalIncorruptibleSharpQuestions()
...
function buildAnsweredList(): AnsweredQuestion[] {
  ...
  for (const cat of INCORRUPTIBLE_SHARP_CATEGORIES) {
    for (const q of cat.questions) {
      ...
      // r93bb: prepend plan-derived suggestion text to the answer when selected — gives the
      // synthesis pipeline access to the plan-aware content with highest provenance.
      ...
      out.push({
        categoryId:                cat.id as IncorruptibleCategory,
        questionId:                q.id,
        category:                  cat,
        answer:                    fullAnswer,
...
function synthesiseAndApply(): void {
  ...
  // r93dd: pass spec so V.-needing fix types resolve against the bound spec instead of
  // silently failing with triggeredBy='plan-level' that no V. matches.
  const findings  = synthesiseIncorruptibleFindings(answered, props.spec ?? null)
  if (findings.length === 0) return
  emit('synthesise-findings', findings)
}

/** r93ee — single-question Apply: synthesise + apply ONLY this question's answer.
 *  Per Tom Gilb 2026-06-11 "not at bottom or with question, i see it attopnow". */
function synthesiseAndApplyOne(catId: string, qId: string): void {
  const cat = INCORRUPTIBLE_SHARP_CATEGORIES.find(c => c.id === catId)
  if (!cat) return
  const qa = getQA(catId, qId)
  const hasInput = qa.answer.trim().length > 0 || qa.selectedSuggestionIndexes.length > 0 || qa.planDerivedSelected
  if (!hasInput) return
  // r93bb: prepend plan-derived suggestion text when selected
  ...
  const single: AnsweredQuestion[] = [{
    categoryId:                catId as IncorruptibleCategory,
    questionId:                qId,
    category:                  cat,
    answer:                    fullAnswer,
    selectedSuggestionIndexes: qa.selectedSuggestionIndexes,
  }]
  const findings = synthesiseIncorruptibleFindings(single, props.spec ?? null)
  if (findings.length === 0) return
  emit('synthesise-findings', findings)
}
...
// build the UniversalSharpExportInput from this panel's native answer state.
function _buildExportInput(): UniversalSharpExportInput {
  return {
    panelName: 'Incorruptible Sharpening',
    planName:  props.planTitle || 'Untitled Plan',
    subtitle:  'Q&A across 6 Incorruptible categories — Ries Figure 5.1 four-pillar framework + Gilb canonical principles.',
    sections:  INCORRUPTIBLE_SHARP_CATEGORIES.map(cat => ({
      headline:  cat.label,
      ...
      items:     cat.questions.map(q => {
        ...
        // Append plan-derived suggestion if present
        const planSuggestion = planDerivedSuggestions.value.get(`${cat.id}|${q.id}`)
        if (planSuggestion) {
          suggestions.push({
            text:   planSuggestion,
            ticked: !!qa.planDerivedSelected,
            source: 'Plan-derived',
          })
        }
        return {
...
async function copyAllAnswers(): Promise<void> {
  await copyUniversalSharp(_buildExportInput())
}
async function emailAllAnswers(): Promise<void> {
  await emailUniversalSharp(_buildExportInput())
}

// Provenance badge metadata — matches existing AISource palette
function provenanceBadge(source: string): { label: string; cls: string } {
  switch (source) {
    ...
    case 'gilb':      return { label: 'Gilb/Ries', cls: 'bg-rose-100 text-rose-900 ring-1 ring-rose-300' }
...
// Reference props.spec / props.planTitle so they're not unused
void props.spec
</script>
...
<template>
  <Teleport to="body">
    ...
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Incorruptible Sharpening — question-and-answer-driven strategic-resilience refinement"
    >
      <div
      ...
      >
        ...
        <div class="flex items-center gap-3 px-5 py-4 bg-gradient-to-r from-slate-900 via-indigo-900 to-slate-900 shrink-0">
          ...
          <button
            ...
            title="Clear all your Sharpening answers for this plan. Answers persist to localStorage across sessions; this wipes them. Confirmation required."
            @click="clearAllAnswers"
          >🗑 Clear</button>
          <!-- r41 v283 (Tom Gilb 2026-06-22 "All sharpening answers must be exportable") — Copy + Email buttons -->
          <button
            type="button"
            class="px-2.5 py-1.5 rounded bg-indigo-600 hover:bg-indigo-500 text-white text-[11px] font-semibold whitespace-nowrap transition-colors ring-1 ring-indigo-400"
            title="📋 Copy all Sharpening Q&A across 6 categories as colourful HTML (paste into Mail / Notes / Keynote)"
            @click="copyAllAnswers"
          >📋 Copy</button>
          <button
            type="button"
            class="px-2.5 py-1.5 rounded bg-indigo-700 hover:bg-indigo-600 text-white text-[11px] font-semibold whitespace-nowrap transition-colors ring-1 ring-indigo-500"
            title="📧 Email all Sharpening Q&A — opens Mail.app pre-filled (paste colour version with ⌘V)"
            @click="emailAllAnswers"
          >📧 Email</button>
          <button
            type="button"
            ...
            :title="canSynthesise
              ? `Synthesise ${totalAnswered} answered question(s) into Incorruptible findings, then apply via the standard Accept-Fix pipeline (Source-stamping + Undo all preserved)`
          ...
          <CloseDot
            variant="on-dark"
            size="lg"
            aria-label="Close Incorruptible Sharpening"
            title="Close Incorruptible Sharpening — your answers persist for this session"
            @click="emit('close')"
          />
        </div>
        ...
        <div class="flex-1 min-h-0 flex">
          ...
          <nav
            ...
            aria-label="Incorruptible Sharpening categories"
          >
            <ul class="py-2">
              <li
                v-for="cat in INCORRUPTIBLE_SHARP_CATEGORIES"
                :key="cat.id"
              >
                <button
                  type="button"
                  class="w-full flex items-start gap-2.5 px-4 py-3 text-left transition-colors hover:bg-slate-100"
                  :class="activeCategoryId === cat.id ? 'bg-white border-r-4 border-indigo-600' : ''"
          ...
          <section class="flex-1 min-h-0 overflow-y-auto px-6 py-5 space-y-6" :aria-label="`${activeCategory.label} questions`">
            ...
            >
              ...
              <div class="space-y-2">
                ...
                <!-- r93bb Phase 2b: plan-derived suggestion (highest provenance) — renders ABOVE
                     the static suggestions when the deterministic engine has context for this
                     specific question against the bound spec. -->
                <ul class="space-y-2">
              ...
              <div>
                ...
                <textarea
                  ...
                  class="w-full min-h-[80px] text-[13px] border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-400 focus:outline-none transition-all"
                  :title="`Free-text answer to: ${q.text}`"
                  @input="setAnswer(activeCategory.id, q.id, ($event.target as HTMLTextAreaElement).value)"
                ></textarea>
              </div>

              <!-- r93ee — per-question Apply button. Tom Gilb 2026-06-11 "not at bottom or with
                   question, i see it attopnow" — answers each apply individually so Tom can iterate
                   one-question-at-a-time without committing every other answer in the panel. -->
            ...
            <!-- r93ee — bottom mirror of the header "Synthesise & Apply" button per DD-014
                 Top-and-Bottom Navigation Mirror rule. After a long category of questions,
                 the user shouldn't have to scroll back to the top to apply all answers. -->
            <div
