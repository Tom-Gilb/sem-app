...
describe('SpecOwnerPanel.vue — auto-save tab-switch bug fix (2026-05-12)', () => {
  ...
  function mountWithFreshModel(initialTab: 'owners' | 'planners' | 'scribes' = 'planners') {
    ...
    const wrapper = mount(SpecOwnerPanel, {
  ...
  it('renders the renamed header "Spec Responsibilities"', () => {
    const { wrapper } = mountWithFreshModel()
    expect(wrapper.text()).toContain('Spec Responsibilities')
