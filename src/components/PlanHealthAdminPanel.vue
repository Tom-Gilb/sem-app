...
import {
  ...
  type AIExpert,
} from '../composables/usePlanHealth'
...
import PlanHealthBadge from './PlanHealthBadge.vue'
import { copyPlanHealthReport, emailPlanHealthReport } from '../composables/usePlanHealthExport'
...
const ph = usePlanHealth(props.planModelId)
...
function toggleExpertEnabled(expertId: string, on: boolean): void {
  ...
  if (on) void undefined // auto-activates ai-experts group in usePlanHealth
}
  ...
  >
    ...
    <div class="px-5 py-3 flex items-center gap-3 shrink-0
      ...
      <div class="flex-1 min-w-0">
        <h2 class="text-sm font-bold tracking-wide">⚙️ Plan Health Administration</h2>
      ...
      >📊 Status</button>
      <CloseDot variant="on-dark" aria-label="Close Plan Health Administration" @click="emit('close')" />
    </div>
    ...
    <ScrollContainer outer-class="flex-1 min-h-0 relative" inner-class="h-full px-5 py-4 space-y-4">
      ...
      <div class="rounded-lg border border-cyan-200 bg-cyan-50/40 overflow-hidden">
        ...
        >
          ...
          <span>📜 Plan Health Record Administration Specification</span>
