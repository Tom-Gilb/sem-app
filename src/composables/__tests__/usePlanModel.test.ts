...
describe('usePlanModel', () => {
  ...
  describe('initPlanModel', () => {
    ...
    it('sets the current model', () => {
      ...
      expect(usePlanModel().currentModel.value?.id).toBe(m.id)
    })
  ...
  describe('bumpPlanVersion', () => {
    it('increments minor: 0.1 → 0.2', () => {
      ...
      bumpPlanVersion()
      expect(usePlanModel().currentModel.value?.version).toBe('0.2')
    })
    ...
    it('rolls over at 0.9 → 1.0', () => {
      ...
      expect(usePlanModel().currentModel.value?.version).toBe('1.0')
    })
    ...
    it('continues past 1.0: 1.0 → 1.1', () => {
      ...
      expect(usePlanModel().currentModel.value?.version).toBe('1.1')
    })
    ...
    it('increments sharpenRounds each time', () => {
      ...
      bumpPlanVersion()
      bumpPlanVersion()
      expect(usePlanModel().currentModel.value?.sharpenRounds).toBe(2)
    })
    ...
    it('updates the spec snapshot when provided', () => {
      ...
      expect(usePlanModel().currentModel.value?.spec.functions[0].description)
        .toContain('Updated')
    })
  ...
  describe('savePlanSnapshot', () => {
    it('updates the spec without bumping the version', () => {
      ...
      const cur = usePlanModel().currentModel.value!
    ...
    it('does not increment sharpenRounds', () => {
      ...
      expect(usePlanModel().currentModel.value?.sharpenRounds).toBe(0)
    })
    ...
    it('updates updatedAt', async () => {
      ...
      expect(usePlanModel().currentModel.value!.updatedAt > m.updatedAt).toBe(true)
    })
  ...
  describe('renamePlanModel', () => {
    ...
    it('syncs currentModel when the renamed model is active', () => {
      ...
      expect(usePlanModel().currentModel.value?.name).toBe('Renamed Active')
    })
    ...
    it('does NOT change currentModel when a different model is renamed', () => {
      ...
      expect(usePlanModel().currentModel.value?.name).toBe('Second')
    })
  ...
  describe('deletePlanModel', () => {
    ...
    it('clears currentModel when the active model is deleted', () => {
      const m = initPlanModel(makeSpec(), 'Active')
      deletePlanModel(m.id)
      expect(usePlanModel().currentModel.value).toBeNull()
    })
    ...
    it('leaves currentModel intact when a different model is deleted', () => {
      ...
      expect(usePlanModel().currentModel.value?.id).toBe(m2.id)
    })
  ...
  describe('importPlanModel', () => {
    ...
    it('sets the imported model as current', () => {
      ...
      expect(usePlanModel().currentModel.value?.id).toBe(result!.id)
    })
